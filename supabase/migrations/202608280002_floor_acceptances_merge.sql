-- Issue #23 data-safety guard: floor-level acceptance signatures are protected
-- from stale snapshot omission. This migration only replaces merge functions;
-- it defines merge behavior only and does not touch business rows.
...
declare
  ...
  ids text[];
  protected_collection boolean := p_path ~ '(^|\.)(surveys|works|defects|acceptances|journals|events|floorAcceptances)$';
begin
  ...
  if jsonb_typeof(p_base) = 'object' and jsonb_typeof(p_local) = 'object' and jsonb_typeof(p_remote) = 'object' then
    ...
    for k in select distinct key from (
      ...
      if k not in ('surveys', 'works', 'defects', 'acceptances', 'journals', 'events', 'floorAcceptances') then
