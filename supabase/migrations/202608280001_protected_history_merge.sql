-- Data-safety guard: stale snapshots cannot delete historical entity records by omission.
-- This migration defines merge semantics only; applying it is a separate production operation.
...
declare
  ...
  ids text[];
  protected_collection boolean := p_path ~ '(^|\.)(surveys|works|defects|acceptances|journals|events)$';
begin
  ...
  if jsonb_typeof(p_base) = 'object' and jsonb_typeof(p_local) = 'object' and jsonb_typeof(p_remote) = 'object' then
    ...
    for k in select distinct key from (
      ...
      if k not in ('surveys', 'works', 'defects', 'acceptances', 'journals', 'events') then
